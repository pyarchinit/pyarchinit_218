...
class generate_reperti_pdf:
	...
	def build_box_labels_Finds(self, records):
		styleSheet = getSampleStyleSheet()
		styNormal = styleSheet['Normal']
		styBackground = ParagraphStyle('background', parent=styNormal, backColor=colors.pink)
		styH1 = styleSheet['Heading1']
		...
		table_data = []
		for i in range(len(records)):
			exp_index = Box_labels_Finds_pdf_sheet(records[i])
			table_data.append(exp_index.getTable())
		...
		doc.build(lst, canvasmaker=None) #lst, canvasmaker=NumberedCanvas_FINDSindex)
